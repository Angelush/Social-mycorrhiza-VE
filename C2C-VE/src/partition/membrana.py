...
def admitir(interaccion: dict) -> dict:
    ...
    if sala == 'don_comunal':
        # PROHIBIR instrumentos de mercado Y claves de libro de reciprocidad
        found, match_key = _contains_forbidden_key(carga, MARKET_KEYS)
        if found:
            raise ErrorDeBrechaMembrana(
                f"Instrumento de mercado detectado en la sala don_comunal: clave {match_key!r}"
            )
        found, match_key = _contains_forbidden_key(carga, RECIPROCITY_LEDGER_KEYS)
        if found:
            raise ErrorDeBrechaMembrana(
                f"Clave de libro de reciprocidad detectada en la sala don_comunal: clave {match_key!r}"
            )
    elif sala == 'igualdad':
        # PROHIBIR solo instrumentos de mercado
        found, match_key = _contains_forbidden_key(carga, MARKET_KEYS)
        if found:
            raise ErrorDeBrechaMembrana(
                f"Instrumento de mercado detectado en la sala igualdad: clave {match_key!r}"
            )
