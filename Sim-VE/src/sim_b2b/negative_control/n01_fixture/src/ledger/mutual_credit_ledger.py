"""N-01 negative-control copy of B2B-VE/src/ledger/mutual_credit_ledger.py — DELIBERATELY BROKEN.

Full copy of the real VE ledger with exactly one surgical change, marked "# N-01 PLANT": the
`clearing_applied` handler's independent re-verification that every member's net delta across
the settlement batch is zero is disabled. With the conservation assert already disabled in this
fixture's clearing_solver.py, disabling this one too is what lets a non-conserving clearing
proposal COMMIT silently (ST6: a plant the SUT catches for you tests its own self-defence, not
the harness's independent oracle). The global zero-sum-of-balances assert is left untouched
(clearing_applied never writes balance_cents). The VE `proposal_moneda` gate (TB.8b) is ALSO
left untouched: the corrupted proposal carries the cell's own moneda, so that gate passes
legitimately — it guards the unit of account, not conservation. TS.1: re-derived from B2B-VE.

Never imported by anything outside the negative-control test suite. `B2B-VE/` is untouched.
"""
import json
import hashlib
import copy
