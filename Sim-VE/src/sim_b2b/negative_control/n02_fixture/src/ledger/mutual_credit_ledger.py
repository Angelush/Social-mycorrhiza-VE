"""N-02 negative-control copy of B2B-VE/src/ledger/mutual_credit_ledger.py — DELIBERATELY BROKEN.

Full copy of the real VE ledger that silently CLAMPS an over-limit obligation/settlement
amount to whatever headroom remains, marked "# N-02 PLANT" at each point of difference from
../../../../../B2B-VE/src/ledger/mutual_credit_ledger.py. Deliberately NOT a naive balance
clamp (audit V3): both sides of every affected op are shrunk to the SAME clamped effective
amount, so the op stays zero-sum and no assert in this file fires; the EVENT payload is also
rewritten to the clamped amount, so the log looks self-consistent. The only way to see the
discrepancy is to compare committed vs requested — exactly what sim_b2b/track_a.py's credit
bound oracle re-derives. TS.1: re-derived from B2B-VE (same plant as the upstream fixture).
