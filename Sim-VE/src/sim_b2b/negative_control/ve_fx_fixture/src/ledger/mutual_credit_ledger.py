"""TS3 ve_fx negative-control copy of B2B-VE ledger — DELIBERATELY BROKEN (silent FX leak).

One surgical plant, marked "TS3 SILENT PLANT": apply_clearing records the day's exchange-rate
gap in the committed clearing_applied payload. See the plant comment for why no self-guard
fires. Never imported outside the negative-control suite; B2B-VE/ is untouched.
"""
import json
import hashlib
import copy
...
def apply_clearing(state: dict, proposal: dict, ratified_by: str, ts: int) -> tuple[dict, dict]:
    ...
    # TS3 SILENT PLANT (ve_fx): the engine itself writes the day's FX gap into the COMMITTED
    # event payload. Silent because every self-guard looks elsewhere: the ledger's _TASA_KEYS
    # scanner guards INPUT params (create_cell), not what the engine writes into its own log;
    # the proposal_moneda gate passes (the moneda is the legal one); proposal_hash still
    # matches (the plant is OUTSIDE `proposal`); L1 is intact (not a cent moves). Only an
    # independent scan of the emitted events — fx_irrepresentable — can see it.
    payload = {
        "proposal": proposal,
        "proposal_hash": proposal_hash,
        "ratified_by": ratified_by,
        "tasa_referencia": {"bcv": 72574, "paralelo": 81483}
    }
