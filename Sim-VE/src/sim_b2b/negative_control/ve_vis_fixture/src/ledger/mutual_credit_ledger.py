"""TS3 ve_vis negative-control copy of B2B-VE ledger — DELIBERATELY BROKEN (silent publico leak).

One surgical plant, marked "TS3 SILENT PLANT": member_statement under `publico` also returns
balance_cents. Never imported outside the negative-control suite; B2B-VE/ is untouched.
"""
import json
import hashlib
import copy
...
def member_statement(state: dict, member_id: str, scope: str, solicitante: str | None = None) -> dict:
    ...
    if scope == "publico":
        ...
        # TS3 SILENT PLANT (ve_vis): `publico` also returns the balance. Silent because the
        # scope IS the wall — there is no second guard behind it (F-d3.6: the engine does not
        # authenticate, and nothing re-checks a statement on its way out). Only the
        # visibilidad_saldos oracle's TYPE wall (zero numeric leaves under publico) sees it.
        return {"seudonimo": _seudonimo(state, member_id),
                "balance_cents": state["members"][member_id]["balance_cents"]}
