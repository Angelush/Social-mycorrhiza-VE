...
class RoundConfig:
    ...
    seed: int
    # --- TS.1 (VE wire) ---
    # `modo` (TA.4): mandatory-by-harness envelope key on every module call. The SUT guards it
    # per capa (deployment posture); the harness always sends it so the VE surface is exercised.
    modo: str = "paz"
    # `moneda` (TA.6): Capa 4 requires it on every campaign envelope; never per-pledge mixing.
    moneda: str = "USD"
