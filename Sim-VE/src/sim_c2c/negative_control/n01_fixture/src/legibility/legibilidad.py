...
def consultar(request: dict) -> dict:
    ...
    # ===================================================================================
    # N-01 SILENT PLANT (negative control — NOT in the real C2C-VE source). A per-node
    # 'reachability' scalar: a god-view centrality table keyed by person token. It carries
    # NO forbidden substring (reachability is not in FORBIDDEN_KEYS, English or castellano),
    # so the module's own surveillance scan — which in any case only scans the INPUT —
    # never sees it. This is exactly the C2C README's named trap: a scalar the blacklist
    # cannot catch. Track A must catch it structurally (out-of-schema per-person field),
    # or the oracle is blind.
    _reachability = {node: round(1.0 / (d + 1), 3) for node, d in dist_to_target.items()} \
        if consultante != objetivo else {}
