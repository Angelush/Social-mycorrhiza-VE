"""NI-02 negative-control copy of B2B-VE/src/ledger/mutual_credit_ledger.py — DELIBERATELY BROKEN.

Full copy of the real VE ledger with one surgical plant (marked "NI-02 SILENT PLANT"): the
obligation membership guard auto-registers foreign parties instead of raising, enabling silent
cross-cell contagion (B2B inv. 6). TS.1: re-derived from B2B-VE. `B2B-VE/` is untouched.
"""
import json
import hashlib
import copy
...
def _apply(state: dict | None, kind: str, payload: dict, ts: int) -> tuple[dict, dict]:
    ...
    if kind == "member_added":
        ...
        # ===================================================================================
        # NI-02 SILENT PLANT (negative control — NOT in the real B2B-VE source). The membership
        ...
        # auto-registered member has balance 0, status active and wide bounds, so they still
        # pass — including the VE `_ESTADOS_OPERATIVOS` check), which is exactly why the SUT's
        # own conservation checks do NOT self-catch this — only Track A's independent roster
        # reconstruction (F-CC) does.
        _WIDE = 10 ** 12
        for _party in (debtor, creditor):
            if _party not in new_state["members"]:
                new_state["members"][_party] = {
                    "turnover_cents": 0, "credit_min_cents": -_WIDE,
