...
# TS.1: the VE membrana's bilingual sets, duplicated verbatim (never imported)
_VALUE_KEYS = ['price', 'precio', 'cost', 'costo', 'coste', 'fee', 'tarifa',
    ...
    def measure(self, trace: Sequence[TraceEvent]) -> IntegrityReport:
        ...
        for e in trace:
            ...
            if v1 is Verdict.PASS and r.kind == "debt_to_gift":
                if not isinstance(r.output, Rejected) and r.output.get("admitido") is True:
                    payload = r.request.get("carga", {})
