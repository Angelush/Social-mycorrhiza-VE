...
class IntegratedWorld(World):
    ...
    def add_vouch(self, frm: str, to: str, cell_id: str) -> None:
        self.vouches.append({"de": frm, "a": to, "celula_id": cell_id, "expira_en": None})
    ...
    def adjudicate(self, actor_id: str, proposal: "Proposal") -> object:
        ...
        if isinstance(proposal, C2CGift):
            try:
                out = self.c2c.admitir({
                    "sala": self.c2c_modes.get(proposal.cell_id, "don_comunal"),
                    "celula_id": proposal.cell_id, "interaccion_id": proposal.interaction_id,
                    "participantes": list(proposal.participants), "carga": dict(proposal.payload),
                    "modo": "paz"})
        ...
        if isinstance(proposal, LeakDebtToGiftRoom):
            ...
            request = {"sala": "don_comunal", "celula_id": proposal.c2c_cell_id,
                       "interaccion_id": proposal.interaction_id, "participantes": [actor_id],
                       "carga": {"settlement_amount_cents": amount, "ref": proposal.obligation_id},
                       "modo": "paz"}
            try:
                output = self.c2c.admitir(dict(request))
            except self.c2c.MembraneBreachError as exc:
        ...
        if isinstance(proposal, ScoreToCredit):
            query_req = {"consultante": proposal.c2c_asker, "objetivo": proposal.c2c_target,
                         "celula_id": proposal.c2c_cell_id, "ahora": _iso(self.tick),
                         "saltos_max": proposal.max_hops, "modo": "paz",
                         "grafo": {"avales": [dict(v) for v in self.vouches],
                                   "hechos": [dict(f) for f in self.facts]}}
            answer = self.c2c.consultar(query_req)
            # derive a B2B credit bound from the SOCIAL answer — the forbidden move
            reachable = answer["desde_tu_posicion"]["alcanzable"]
