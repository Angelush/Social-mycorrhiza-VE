"""N-01 negative-control copy of B2B/src/ledger/mutual_credit_ledger.py — DELIBERATELY BROKEN.

Full copy of the real ledger with exactly one surgical change, marked "# N-01 PLANT" at the
point it differs from ../../../../B2B/src/ledger/mutual_credit_ledger.py: the
`clearing_applied` handler's own independent re-verification that every member's net delta
across the whole settlement batch is zero is disabled. This is the SECOND of the SUT's three
self-checks (audit V2) -- with the conservation assert already disabled in this fixture's
clearing_solver.py, disabling this one too is what lets a non-conserving clearing
proposal actually COMMIT silently, rather than being caught by the ledger's own defence at
apply time (the ST6 failure mode this negative control exists to avoid: a plant the SUT
catches for you tests its own self-defence, not the harness's independent oracle).

The ledger's OTHER self-check -- the global zero-sum-of-balances assert that runs after every
op -- is left untouched and does not need disabling for this specific plant: clearing_applied
never writes to balance_cents at all (only to the obligations dict), so a clearing-time cent
drop cannot trip a balance-based check regardless.
...
def _apply(state: dict | None, kind: str, payload: dict, ts: int) -> tuple[dict, dict]:
    ...
    if kind == "member_added":
        ...
        # N-01 PLANT: the real ledger re-verifies here that every member's net delta across
        # this whole settlement batch is exactly zero, and rejects the proposal otherwise --
        # this is the ledger's OWN independent defence against exactly the kind of uneven,
        # non-conserving reduction this fixture's clearing_solver.py now produces. Disabled so the
        # corrupted proposal commits instead of being rejected at apply time (audit V2).
        for ob_id, reduce_amount in aggregated.items():
            ob = new_state["obligations"][ob_id]
