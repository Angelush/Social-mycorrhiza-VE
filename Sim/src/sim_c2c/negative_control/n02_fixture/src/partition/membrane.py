...
def admit(interaction: dict) -> dict:
    ...
    # ===================================================================================
    # N-02 SILENT PLANT (negative control — NOT in the real C2C source). The market/
    # reciprocity scan for the non-market rooms is DISABLED, so an interaction carrying a
    # price/_cents key into a communal_gift or equality_matching room is silently ADMITTED
    # (admitted=True) instead of raising MembraneBreachError. The kula/gimwali wall
    # (invariant 1) is down. Crucially SURGICAL: the surveillance scan (step 2 above) is
    # left intact, so this plant still self-catches a forbidden-shaped payload — only the
