...
def _setup_event(mid: str, stmt: dict) -> TraceEvent:
    return TraceEvent(
        ...
        result={
            ...
            "payload": {
                "member": {
                    "id": mid, "turnover_eur_cents": 10_000_000,
...
def _drive_three_cycle(adapter: B2BAdapter) -> list[TraceEvent]:
    ...
    setup = []
    for mid in ("A", "B", "C"):
        adapter.add_member({"id": mid, "turnover_eur_cents": 10_000_000}, ratified_by="ops", ts=0)
        setup.append(_setup_event(mid, adapter.member_statement(mid)))
...
def test_n01_naive_single_guard_plant_is_self_caught_not_a_valid_gate():
    ...
    for mid in ("A", "B", "C"):
        state, _ = real_ledger.add_member(state, {"id": mid, "turnover_eur_cents": 10_000_000}, ratified_by="ops", ts=0)
...
def test_n02_silent_clamp_is_caught_by_credit_bound_oracle():
    ...
    for mid in ("A", "B"):
        adapter.add_member({"id": mid, "turnover_eur_cents": 10_000_000}, ratified_by="ops", ts=0)
        trace.append(_setup_event(mid, adapter.member_statement(mid)))
