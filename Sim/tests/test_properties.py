...
def test_p02_reproducibility_across_random_seeds(seed, t):
    cfg = RoundConfig(
        ...
        n_firms=6, T=t, clearing_cadence=3, base_turnover_eur_cents=10_000_000,
...
def test_p04_track_separation():
    result = build_campaign(
        RoundConfig(
            ...
            base_turnover_eur_cents=10_000_000, neg_line_bp=1000, pos_line_bp=1000,
...
def test_p05_no_float_ever_touches_a_value_path_amount(amount):
    ...
    adapter.add_member({"id": "A", "turnover_eur_cents": 10_000_000}, ratified_by="ops", ts=0)
    adapter.add_member({"id": "B", "turnover_eur_cents": 10_000_000}, ratified_by="ops", ts=0)

    event = adapter.record_obligation({"id": "o1", "debtor": "A", "creditor": "B", "amount_cents": amount}, ts=0)
    _assert_no_float_amounts(event)
    _assert_no_float_amounts(adapter.member_statement("A"))
    _assert_no_float_amounts(adapter.member_statement("B"))
