...
CFG = RoundConfig(
    actor_mix={}, n_firms=8, T=15, clearing_cadence=5, base_turnover_eur_cents=10_000_000,
...
def _build_cell_with_setup_events():
    ...
    for fid in neighbors:
        adapter.add_member({"id": fid, "turnover_eur_cents": CFG.base_turnover_eur_cents}, ratified_by="ops", ts=0)
        stmt = adapter.member_statement(fid)
        setup_events.append(
            TraceEvent(
                ...
                result={
                    ...
                    "payload": {
                        "member": {
                            "id": fid,
                            "turnover_eur_cents": CFG.base_turnover_eur_cents,
    ...
    def ev(mid, cmin, cmax):
        return TraceEvent(
            ...
            result={
                "kind": "member_added",
                "payload": {
                    "member": {"id": mid, "turnover_eur_cents": 10_000_000, "credit_min_cents": cmin, "credit_max_cents": cmax},
                    "ratified_by": "ops",
                },
            },
        )
    ...
    setup = _setup_two_members() + [
        TraceEvent(
            ...
            result={
                "kind": "member_added",
                "payload": {
                    "member": {"id": "C", "turnover_eur_cents": 10_000_000, "credit_min_cents": -1_000_000, "credit_max_cents": 1_000_000},
                    "ratified_by": "ops",
                },
            },
        )
