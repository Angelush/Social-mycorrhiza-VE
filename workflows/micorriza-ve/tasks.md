...
| TP.2 | Sincronización con upstream: `git fetch upstream && git merge` documentado; divergencias al log de conflictos de `context.md` | E4: el fork no escribe upstream. **Primer sync ejecutado 2026-07-17** (merge `94605fe`, contenido byte-idéntico, log en `B2B/workflows/micorriza-ve/context.md` §4) — la fila sigue abierta: es continua |
| TP.3 | Resolver E2 (alcance castellanización B2B) — bloquea TB.1 | humano. ✅ **Decidida 2026-07-15** — documentada en `B2B/workflows/micorriza-ve/context.md` §"E2 (TP.3)"; el sub-bundle TB.1 (`3ef5054`) la incorpora y la Fase 2 cerró con ella |
